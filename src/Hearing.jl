...
"""
    sim_ihc()
"""
function sim_ihc(px, cf, nrep, tdres, totalstim, cohc, cihc, species, ihcout)
    ccall((:IHCAN, libihc), Cvoid, (Ptr{Cdouble}, Cdouble, Cint, Cdouble, Cint, Cdouble, Cdouble, Cint, Ptr{Cdouble}),
          px, cf, nrep, tdres, totalstim, cohc, cihc, species, ihcout)
end

function NLogarithm(x, slope, asym, cf)
    ccall((:NLogarithm, libihc), Float64, (Float64, Float64, Float64, Float64),
          x, slope, asym, cf)
end
